def baseline_elec_attention(self,elec1,elec2,state_size):
    ...
    att_embed_W1 = tf.Variable(tf.random_uniform([self.config.model6_att_elec_dim, self.embed_att_elec_dim], -0.1,0.1), name='baseline_embed_att_W_elec1')
    att_embed_W2 = tf.Variable(tf.random_uniform([self.embed_att_elec_dim, 1], -0.1,0.1), name='baseline_embed_att_W_elec2')
    att_embed_b1 = tf.Variable(tf.zeros([self.embed_att_elec_dim]), name='baseline_embed_att_b_elec1')
    att_embed_b2 = tf.Variable(tf.zeros([1]), name='baseline_embed_att_b_elec2')

    feat_tensor = tf.reshape(elec_current_time_x, [-1,self.embed_att_elec_dim])

    e = tf.nn.relu(tf.matmul(feat_tensor, self.att_embed_W1)+self.att_embed_b1 )  # [batch * nb_electrodes, embed_att_elec_dim]
    e = tf.matmul(e,self.att_embed_W2)+ self.att_embed_b2  # [batch * nb_electrodes, 1]
    e = tf.reshape(e, [ batch_size, max_nb_channels ])  # [batch , nb_electrodes]
       
    alphas = tf.nn.softmax(e)    
    alphas = tf.reshape(alphas, [ batch_size, max_nb_channels,1 ])

    #attention_list : multply alphas [batch,nb_electrodes,1] with [batch,nb_electrodes,electrode_representation_size]
